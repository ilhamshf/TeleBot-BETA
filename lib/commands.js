   ...
   */
        ...
        if (msg.chat.type === 'supergroup') {
            ...
            if (this.commonFunctions.messageSenderIsAdmin(admins, msg)) {
                ...
                if (!enoughtRights) {
                    ...
                    this.bot.sendMessage(msg.from.id, `Konfigurasi grub* ${msg.chat.title}*\n\n*NOTE*:\n🔴=Dinonaktifkan\n🟢=Diaktifkan`, {
                        parse_mode: 'markdown',
                        reply_markup: kbd
                    });
    ...
    async kickByReply(msg) {
        ...
        if(!msg.reply_to_message){
            await this.bot.sendMessage(msg.from.id, 'Silakan balas pesan pengguna yang ingin Anda tendang');
            return;
        }              
    ...
    async banByReply(msg) {
        ...
        if (!this.commonFunctions.messageSenderIsAdmin(admins, msg)) return;
        if(!msg.reply_to_message){
            await this.bot.sendMessage(msg.from.id, 'Silakan balas pesan pengguna yang ingin Anda ban');
            return;
        } 
        if (!this.commonFunctions.messageSenderIsAdmin(admins, msg.reply_to_message)) {
    ...
    helpCommand(msg) {
        ...
        const text = `*PENTING*\nBot ini hanya dapat berfungsi di supergrup untuk saat ini!\n\nUntuk mengonfigurasi bot di grup Anda, Anda perlu:\n1) Mengundang bot ke grup Anda.\n2) Promosikan dia menjadi admin (aktifkan "Hapus pesan" dan "Blokir pengguna").\n3) Konfigurasi bot dengan mengirim / mengatur langsung ke grup Anda (pesan akan segera hilang).\n\n*Perintah yang tersedia*\n/help -> Menunjukan pesan ini\n/blacklist -> Untuk mendapatkan daftar semua kata yang masuk daftar hitam, jika membalas pesan, pesan itu akan masuk daftar hitam\n/blacklist %word% -> Untuk menambahkan 'kata' ke daftar hitam\n/set\\_hello %pesan yang ingin anda kirim%\n\nSetting pesan halo untuk anggota obrolan baru. Anda dapat menggunakan placeholder $ name, itu akan diganti dengan nama peserta baru.\nPerintah panggilan tanpa pesan untuk menyetel default. Pastikan "Pesan Halo untuk anggota baru" diaktifkan.`;   
    ...
    async actionHandler(groupId,action){
        if(action=='*resetBlacklist'){
            ...
            this.bot.sendMessage(groupId,'Pesan blacklist telah direset oleh admin');
        }
    }
    ...
    startCommand(msg) {
        ...
        this.bot.sendMessage(msg.from.id, 'Selamat datang silahkan ketik /help untuk melihat perintah yang tersedia.');
    }
    ...
    async setHelloCommand(msg, match) {
        ...
        if (group) {
            ...
            if (message)
                this.bot.sendMessage(msg.chat.id, `_Pesan selamat datang grub_ *${group.title}* _telah diatur menjadi: _\n${message}`, { parse_mode: 'markdown' });
            else
    ...
    async blacklistCommand(msg, match) {
        ...
        if(!match[2]){
            this.bot.sendMessage(msg.chat.id,'Gunakan /blacklist <kata yang dilarang> untuk melarang pesan dari semua anggota untuk dikirimkan ke grup');
            return;
        }   
        const word = match[2];
        if (group) {
    ...
    getSetOfKeys(groupConfig) {
        return {
            inline_keyboard: [
                [{
                    text: `${this.switcher(groupConfig.joinedMsg)} Hapus pesan 'bergabung' `,
                    callback_data: `config#${groupConfig.groupId}#joinedMsg`
                }], [{
                    text: `${this.switcher(groupConfig.pinnedMsg)} Hapus pesan 'pin' `,
                    callback_data: `config#${groupConfig.groupId}#pinnedMsg`
                }], [{
                    text: `${this.switcher(groupConfig.arabicMsg)} Hapus pesan arab `,
                    callback_data: `config#${groupConfig.groupId}#arabicMsg`
                }], [{
                    text: `${this.switcher(groupConfig.urlMsg)} Hapus messages dengan url`,
                    callback_data: `config#${groupConfig.groupId}#urlMsg`
                }], [{
                    text: `${this.switcher(groupConfig.deleteCommands)} Hapus pesan dengan perintah`,
                    callback_data: `config#${groupConfig.groupId}#deleteCommands`
                }], [{
                    text: `${this.switcher(groupConfig.deleteBlacklist)} Hapus pesan Blacklisted`,
                    ...
                    text: `${this.switcher(groupConfig.helloMsg)} Pesan selamat datang untuk member barur`,
                    callback_data: `config#${groupConfig.groupId}#helloMsg`
                }], [{
                    text: `${this.switcher(groupConfig.allowAdminsToConfigure)} Ijinkan admin untuk mengkonfigurasi bot`,
